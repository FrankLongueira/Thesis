...
"""
# Generate test set
audio_time_series_test, fs = ap.load_audio( audio_folder_path, audio_filename = "RichardKilmer_5min.wav")
...
scipy.io.wavfile.write( filename = parent_cwd + "/Audio_Files/Test_Files/" + "RichardKilmer_1min.wav", rate = fs, data = audio_time_series_test.astype('int16')[0:(60*fs)])
scipy.io.wavfile.write( filename = parent_cwd + "/Audio_Files/Test_Files/" + "RichardKilmerNoisy_5dB_1min.wav", rate = fs, data = audio_time_series_test_noisy.astype('int16')[0:(60*fs)])
...
scipy.io.wavfile.write( filename = parent_cwd + "/Audio_Files/Test_Files/" + model_name + "_RichardKilmerFiltered_5min.wav", rate = fs, data = test_filtered)
scipy.io.wavfile.write( filename = parent_cwd + "/Audio_Files/Test_Files/" + model_name + "_RichardKilmerFiltered_1min.wav", rate = fs, data = test_filtered[0:(60*fs)])
os.chdir(parent_cwd + "/Audio_Files/Test_Files")
call( ["./PESQ", "+16000", "RichardKilmer_1min.wav", model_name + "_RichardKilmerFiltered_1min.wav"] )
call( ["./PESQ", "+16000", "RichardKilmer_1min.wav", "RichardKilmerNoisy_5dB_1min.wav"] )
